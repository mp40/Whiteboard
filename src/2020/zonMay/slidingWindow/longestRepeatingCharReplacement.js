...
export const characterReplacement = (s, k) => {
    ...
    for(let windowEnd = 0; windowEnd < s.length; windowEnd++) {
        cache[s[windowEnd]] = cache[s[windowEnd]] + 1 || 1;
        mostFrequent  = Math.max(mostFrequent, cache[s[windowEnd]]);

        while(windowEnd - windowStart + 1  - mostFrequent > k) {
            cache[s[windowStart]] -= 1
            windowStart++
        }
