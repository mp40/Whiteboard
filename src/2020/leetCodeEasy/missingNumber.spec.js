...
describe('find missing number', () => {
    it('should work for for first example', () => {
        const input = [3,0,1];
        expect(missingNumber(input)).toBe(2)
    })
    it('should work for for second example', () => {
        const input = [9,6,4,2,3,5,7,0,1];
        expect(missingNumber(input)).toBe(8)
    })
    it('should work when "1" is missing from length 2 array', () => {
        const input = [0];
        expect(missingNumber(input)).toBe(1)
    })
    it('should work when "0" is missing from length 1 array', () => {
        const input = [1];
        expect(missingNumber(input)).toBe(0)
    })
    it('should work when "0" is missing from length 0 array', () => {
        const input = [];
        expect(missingNumber(input)).toBe(0)
    })
